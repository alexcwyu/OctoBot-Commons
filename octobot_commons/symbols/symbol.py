...
class Symbol:
    ...
    def parse_symbol(self, symbol_str: str):
        ...
        if self.settlement_separator in symbol_str:
            ...
            if option_type_str == "P":
                self.option_type = octobot_commons.enums.OptionTypes.PUT
            elif option_type_str == "C":
                self.option_type = octobot_commons.enums.OptionTypes.CALL
            else:
    ...
    def merged_str_symbol(
        ...
        """
        market = f"{self.base}{market_separator}{self.quote}"
        settlement = f"{settlement_separator}{self.settlement_asset}"
        if self.strike_price and self.identifier and self.option_type:
            ...
            return f"{market}{settlement}{option_separator.join(details)}"
        if self.settlement_asset:
            return f"{market}{settlement}"
        return market
