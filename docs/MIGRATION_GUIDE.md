# Migration Guide: OctoBot-Commons
...
### 3. Source Layout
- **Layout**: `src/` layout
- **Package moved**: `octobot_commons/` -> `src/octobot_commons/`
- **Import unchanged**: `import octobot_commons` still works

If upstream adds files to the old location, move them to `src/octobot_commons/`.
...
known-first-party = ["octobot_commons"]
```
...
3. **Check source layout**: If upstream adds new modules to the old path, move them to `src/octobot_commons/`.
4. **Re-lock**: Run `uv lock` to update `uv.lock` with any new/changed dependencies.
5. **Verify**: Run `uv sync && uv run python -c "import octobot_commons" && uv run pytest` (if tests exist).

## Quick Commands

```bash
uv sync                                       # Install all deps
uv run python -c "import octobot_commons"      # Verify import
